...
function Pull-FromMediaWiki {
    ...
    if ($LASTEXITCODE -eq 0) {
        ...
        if ($LASTEXITCODE -eq 0) {
            ...
            Write-Host ""
            Write-Host "⚠ Merge conflict. Please resolve conflicts manually." -ForegroundColor Yellow
            return $false
        }
    } else {
        Write-Host ""
...
function Push-ToMediaWiki {
    ...
    git push wiki "${CURRENT_BRANCH}:refs/heads/master" 2>&1 | Write-Host
    
    if ($LASTEXITCODE -eq 0) {
        Write-Host ""
        Write-Host "✓ Successfully pushed to MediaWiki!" -ForegroundColor Green
        return $true
    } else {
        Write-Host ""
        Write-Host "✗ Failed to push to MediaWiki" -ForegroundColor Red
        return $false
    }
}
...
switch ($choice) {
    ...
    "2" {
        ...
        if (Push-ToMediaWiki) {
            Write-Host ""
            Write-Host "✓ Changes are now live on MediaWiki!" -ForegroundColor Green
        }
    }
    "3" {
        # Full sync
            ...
            if (Push-ToMediaWiki) {
                ...
                if ($remotes -contains "origin") {
                    ...
                    if ($LASTEXITCODE -eq 0) {
                        ...
                        Write-Host ""
                        Write-Host "⚠ Failed to push to GitHub. You may need to set up the remote first:" -ForegroundColor Yellow
                        Write-Host "  git remote add origin [your-github-repo-url]" -ForegroundColor White
                    }
                } else {
                    Write-Host ""
                    Write-Host "⚠ No GitHub remote found. Add it with:" -ForegroundColor Yellow
                    Write-Host "  git remote add origin [your-github-repo-url]" -ForegroundColor White
